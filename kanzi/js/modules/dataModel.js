...
// 読み例の各要素は { 語, 読み, 例文, 確認状態}（2026-09-02新設。単漢字読みクイズ用。語は対象文字列＝
// 単漢字そのもの、または送り仮名・数え方の「つ」等を含む語。熟語の読みクイズ（複数の漢字にまたがる語の
// 例文を使う）とは別に、その字1字だけの読みを対象級の語彙のみで自然に問うための例文。現状10級のみ投入済み。
// buildReadingQuiz（quiz.js）が対象級にこのデータがあれば熟語プールと混ぜて出題する。CLAUDE.md 2章参照）。
// 読み_確認状態・書き_確認状態は漢字1字単位（音読み・訓読みのセット全体に対するレビュー用、js/modules/devReview.jsのREVIEW_STATUSESと対応）。
export const KANJI_COLUMNS = ['ID', '漢字', '級', '学年', '画数', '部首', '部首名', '音読み', '訓読み', '意味', '送り仮名例', '読み例', '意味_機械翻訳', '読み_確認状態', '書き_確認状態', '筆順_確認状態'];
