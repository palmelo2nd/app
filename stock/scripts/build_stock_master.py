...
入力: data_j_*.xls（ユーザーが外部から取り込む生データ。既定の置き場所は
      ../../../app_data/stock/input/listed_companies/）
出力: {output}（既定: ../../../app_data/stock/master.csv）
...
2026-08-26判断: 「コード→銘柄名」の対応を、他のデータ（保有銘柄・売買履歴等）が参照する際の正の基準にしたいため、
出力先に既存のmaster.csvがあれば洗い替えではなく差分マージする（積み上げ型）。
- 入力（今回のJPX一覧）に存在するコードは、名前・市場区分等を最新の内容に更新する（status="listed"、as_ofも更新）
- 入力に存在しないが既存master.csvにあるコード（上場廃止・銘柄コード変更等で一覧から消えたもの）は行を削除せず残し、
  status="listed"だった場合のみ"delisted"に切り替える（それ以外のフィールド・as_ofは最後に確認できた時点のまま変更しない）。
  fetch_prices.pyのload_codes_from_masterは既にstatus=="listed"のみを対象にしているため、この扱いだけで
  株価取得対象からは自然に外れる。
- コードを持たない米国投信等を追加する場合は、source列を "us_fund" 等に分けて管理する想定（未実装）
...
def merge_master(fresh: pd.DataFrame, existing: pd.DataFrame) -> pd.DataFrame:
    """今回のJPX一覧から作った fresh を、既存の master.csv（existing）に積み上げマージする。
    fresh側にあるコードは最新内容で上書きする。existing側にしか無い行のうち、
    source="tse_data_j"（過去のJPX一覧由来）のものだけを「一覧から消えた銘柄」とみなし、
    行を保持したまま status のみ "delisted" に切り替える（listed以外のstatusは変更しない）。
    source が tse_data_j 以外の行（投信名をcodeとして手動登録した行など）はJPX一覧の対象外なので、
    fresh に無くても常にそのまま保持する（delisted化しない）。"""
    fresh_codes = set(fresh["code"])
    stale = existing[~existing["code"].isin(fresh_codes)].copy()
    from_jpx = stale["source"] == "tse_data_j"
    stale.loc[from_jpx & (stale["status"] == "listed"), "status"] = "delisted"
...
def main():
    ...
    parser.add_argument("--input", required=True, help="変換元のExcelファイル（data_j_*.xls。通常は ../../../app_data/stock/input/listed_companies/ 配下）")
    parser.add_argument("--output", default="../../../app_data/stock/master.csv", help="出力先CSVパス")
    ...
    if output_path.exists():
        ...
        before = len(existing)
        master = merge_master(master, existing)
        non_listed = len(master[master["status"] != "listed"])
        print(f"既存master.csv（{before}件）に積み上げマージしました（今回listed{len(master) - non_listed}件＋保持中の非listed{non_listed}件＝計{len(master)}件）")
